...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.standparam = config.standparam
...
class MoeLayer(nn.Module):
    def __init__(self, experts: List[nn.Module], gate: nn.Module, moe_args):
        ...
        # print the number of parameters of the first expert
        print(f'Activated expert has {2*sum(p.numel() for p in self.experts[0].parameters())} parameters')
        self.gate = gate
        self.args = moe_args
...
class MLP(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.c_fc    = nn.Linear(config.n_embd, config.n_embd * config.ffn_ratio, bias=config.bias)
        self.gelu    = nn.GELU()
        self.c_proj  = nn.Linear(config.n_embd * config.ffn_ratio, config.n_embd, bias=config.bias)
